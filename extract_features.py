...
def extract_match_features(match, time_point=None):
    ...
    print '\n'
    print match['players'][6]['kills_log']
    print '\n'
    exit()
    ...
    for player_index, player in enumerate(match['players']):
        ...
        feats += [
            ...
            (player_id + '_abilities', len(filter_events(player['ability_upgrades'], time_point))),
            (player_id + '_obs', len(filter_events(player['obs_log'], time_point)) + len(filter_events(player['sen_log'], time_point))),
        ]
