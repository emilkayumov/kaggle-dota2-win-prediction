...
def extract_match_features(match, time_point=None):
    ...
    abilities = pandas.read_csv('../input/dictionaries/abilities.csv', header=0)
    abilities = numpy.array(abilities)

    extract_abilities_count = [(x[0], x[1]) for x in abilities]
    ...
    for team, team_players in (('radiant', radiant_players), ('dire', dire_players)):
        ...
        for ability_id, ability_name in extract_abilities_count:
            abilities_count = sum([
                1
                for player in team_players
                for entry in filter_events(player['ability_upgrades'], time_point)
                if entry['ability'] == ability_id
            ])
            feats += [
                ('%s_%s_count' % (team, ability_name), abilities_count)
            ]
